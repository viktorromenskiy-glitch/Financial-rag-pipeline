...
## ОБЯЗАТЕЛЬНОЕ ПРАВИЛО: автоматическое сохранение результатов платных/значимых прогонов (добавлено 2026-08-19)

**КАЖДЫЙ значимый результат — в первую очередь любой прогон, использующий реальные платные API-вызовы (`pipeline.cli eval`, `pipeline.cli index`, любой скрипт из `scripts/`), — ОБЯЗАН сохраняться автоматически, в ТОЙ ЖЕ ячейке/скрипте, что его производит. Не отдельным шагом, который можно забыть выполнить.**

Обязательная последовательность, без исключений:

1. Запуск прогона.
2. Жёсткая проверка результата сразу после — `raise`/остановка выполнения, не просто `print`: все ожидаемые файлы существуют, не пусты, число строк/записей соответствует ожидаемому n. Прогон, не прошедший эту проверку, не считается завершённым и не докладывается как готовый.
3. Автоматическое копирование результата на Google Drive — в том же скрипте/ячейке, сразу после проверки шага 2, не отдельным ручным действием.
4. Только после шагов 1-3 результат готов к коммиту в GitHub (коммитит пользователь вручную, как и весь остальной код в этом проекте).

**Почему это правило, а не пожелание — прецедент, обошедшийся в реальные деньги и время:** прогон `pipeline.cli eval` (n=250, полный пайплайн), результат которого (judge accuracy 0.865/0.711/0.780/0.768 по source_dataset) лёг в основу README.md и `tehnicheskoe_zadanie.md`, был выполнен и оплачен (реальные вызовы Claude Sonnet 5 + Cohere Rerank), но нигде не сохранён — ни в Git, ни на Google Drive. Причина: `results/<run_id>/` — путь на эфемерном диске Colab-рантайма, не переживающий отключение сессии, если не скопирован явно. Данные потеряны безвозвратно, для последующего разбора ошибок (Тир 6c общего плана улучшений) потребовался полностью новый платный прогон. Правило выше — прямое следствие этого случая, не абстрактная предосторожность.
