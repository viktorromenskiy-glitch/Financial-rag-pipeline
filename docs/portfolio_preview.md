...
**Оговорка по источнику.** План-2 предполагал использовать как основу готовый ~250-словный черновик от ChatGPT — но точный текст этого черновика был в части переписки, которая к этому пункту уже сжалась (context compaction) и не сохранилась дословно. Пересказывать по памяти то, чего не вижу перед собой, значило бы нарушить тот же принцип "только проверенная информация", который применяется к остальному проекту — поэтому текст ниже был написан заново, из проверенных цифр README (не из ChatGPT-черновика), а не реконструирован по памяти.

**Обновление 2026-08-21 (после повторной загрузки оригинального документа ChatGPT).** Виктор прислал исходный docx с полным техническим аудитом, из которого был синтезирован план-2 — включая тот самый черновик Layer 1, которого не хватало выше. Сверка с ним попутно вскрыла реальную неточность: черновик ChatGPT явно рекомендует вести заголовок с **76.0%** ("the preserved final evaluation run"), а не с 76.8% — потому что 76.8% относится к более раннему прогону, чьи сырые результаты были потеряны при отключении Colab-рантайма (см. `docs/tehnicheskoe_zadanie.md`, раздел 14). Текст ниже при первой публикации этого файла ошибочно унаследовал 76.8% из тогдашнего заголовка README (который сам содержал ту же неточность) — сейчас обе цифры в этом файле и в README (все 3 языка) исправлены на 76.0%, с 76.8% сохранённым только как явно помеченный исторический контекст. Подробности исправления: `docs/tehnicheskoe_zadanie.md`, раздел 20.
...
> Financial-document RAG (text + tables) evaluated end-to-end on 7,300+ real filings: 76.0% answer accuracy, +11-13pp over published retrieval benchmarks, every decision backed by statistical testing (McNemar, power analysis), not intuition.
...
>
> It's tested end-to-end on 7,318 real financial documents and over 23,000 questions — a harder benchmark than the clean-prose demos most RAG portfolios use, because financial answers have to be numerically exact and the source data mixes text with tables. Result: 76.0% answer accuracy (retrieval + reranking + generation + judging), and a retrieval stage that beats the closest published comparison by 11-13 percentage points.
>
