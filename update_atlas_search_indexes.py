...
Usage (Colab, after the usual %cd + secrets-loading cells):
    !python update_atlas_search_indexes.py
"""
...
def main() -> None:
    ...
    print(
        ...
        "next: migrate_embedding_routing.py, then a normal `python -m pipeline.cli eval` run "
        "(or run_migration_and_eval.py, which does both in one step)."
    )
