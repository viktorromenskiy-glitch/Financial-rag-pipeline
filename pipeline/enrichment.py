...
def enrich_document(
    ...
    """Summarizes a single document via the configured summarizer.

    Args:
        summarizer: Summarization client (or a fake implementing SummarizerProtocol).
        context_id: Identifier of the document being summarized.
        raw_content: Raw document text to summarize.

    Returns:
        An EnrichmentResult with context_id and the generated contextual_summary.
    """
...
class EnrichmentCheckpoint:
    ...
    def __init__(self, path: str | Path):
        """Args:
            path: Path to the checkpoint JSONL file (created on first
                append if it doesn't exist).
        """
        self.path = Path(path)

    def load_done(self) -> dict[str, str]:
        """Loads the checkpoint file's contents.

        Returns:
            Mapping from context_id to contextual_summary for every
            document already appended to the checkpoint file, or {} if
    ...
    def append(self, result: EnrichmentResult) -> None:
        """Appends one enrichment result to the checkpoint file, creating
...
def enrich_documents(
    ...
    """Enriches a batch of documents, resuming from checkpoint if provided.

    Args:
        summarizer: Summarization client, or None when enabled=False.
        documents: (context_id, raw_content) pairs, unique per context_id
            (output of dedupe_documents from module 5).
        checkpoint: Checkpoint to resume from and append newly computed
            results to; required when enabled=True.
        enabled: When False (plan Step 2), skips all API calls and
            returns an empty contextual_summary for every document;
            summarizer/checkpoint are not used in that case.

    Returns:
        Mapping from context_id to contextual_summary, covering every
        document in `documents`.

    Raises:
        ValueError: If enabled=True but summarizer or checkpoint is None.
    """
    if not enabled:
