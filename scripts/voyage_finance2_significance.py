...
test, sourcing queries from the FULL T2-RAGBench raw question set (23,088
questions via pipeline.ingestion) rather than the 250-question
eval_subset_250.parquet an earlier draft of this test used.

Why the full question set instead of eval_subset_250.parquet: an earlier,
smaller run (n=200, drawn from eval_subset_250.parquet) found NO result
...
no id/context_id-prefix guessing needed, unlike scripts/voyage_finance2_ab.py's
infer_source().
...
every document in MongoDB from the original scripts/voyage_finance2_ab.py
...
scripts/voyage_finance2_ab.py has already run once):
    !python scripts/voyage_finance2_significance.py
"""
...
from voyage_finance2_ab import BATCH_SIZE, embed_batch, normalize
...
# datasets). Rather than decide on a per-dataset embedding-routing change
# from a nominally-significant-but-correction-fragile result, the practical
# move is to collect more TAT-DQA data first, since it costs only query
# embeddings (near-$0) to do so. 2500 is comfortably below the smallest
# available per-source pool (ConvFinQA has ~3453 raw rows total), so all
# three sources get an equal, maximal-within-budget sample rather than an
...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(
            ...
            f"scripts/voyage_finance2_ab.py first. First few missing: {missing[:5]}"
        )
