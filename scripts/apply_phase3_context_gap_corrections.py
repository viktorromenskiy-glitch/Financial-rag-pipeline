...
def apply_corrections(records: list[dict]) -> list[dict]:
    """Adds a `manual_correction_phase3` field to the 8 Phase 3 reviewed records.
    ...
    Returns:
        A new list of records, each with a `manual_correction_phase3` field
    ...
    for r in records:
        ...
        if qid in CORRECTIONS:
            ...
            r["manual_correction_phase3"] = {
                "reviewed": "internal error-analysis plan, phase 3 (raw_response taxonomy pre-check), 2026-08-23",
                "reason": c["reason"],
...
def cumulative_corrected_stage(r: dict) -> str:
    """Computes the cumulative Phase 1 + 2 + 3 corrected failure stage for one record.
    ...
    Args:
        r: An attribution_results.jsonl record, possibly carrying
            `manual_correction`, `manual_correction_phase2` and/or
            `manual_correction_phase3` fields.
