"""Приоритет 3 из плана adversarial-robustness (внутренние рабочие
материалы, не в этом репозитории; "unanswerable / out-of-corpus вопросы") -
...
В отличие от scripts/reevaluate_phase6_adaptive.py и других eval-скриптов
...
реальной потери чекпоинта раньше в этом проекте, см. внутренние рабочие правила
проекта, раздел 1, п.3); (3) n_retrieved теперь честно отражает размер пула
...
def main() -> None:
    ...
    # в конце - правило раздела 1 п.3 внутренних рабочих правил проекта,
    # тот же фикс, что scripts/reevaluate_phase6_adaptive.py принял после реальной
