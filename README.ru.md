...
Contextual enrichment (Claude Haiku 4.5)	+0.78 п.п. Recall@5	p=0.0164, n=1535 — см. «Известные ограничения» насчёт масштаба
...
Документация
docs/tehnicheskoe_zadanie.md — полная техническая спецификация, каждое решение с измеренным обоснованием, включая вывод per-dataset routing (раздел 3a)
...
Не подтверждено на полном масштабе: эффект contextual enrichment (+0.78 п.п.) и его сочетание с reranker'ом (Recall@5 = 0.980) измерены на уменьшенной подвыборке в 450 документов, не на полном корпусе из 7318. Прогон на полном корпусе подтвердил бы или скорректировал эти числа перед тем, как считать их финальными.
