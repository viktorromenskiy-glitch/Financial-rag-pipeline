...
Утрата и восстановление закоммиченного прогона: прогон n=250 для разбора ошибок (results/error_analysis_250), на числах которого основаны результаты выше, — это повторный прогон. Оригинальный прогон не был сохранён и был безвозвратно утерян при отключении Colab-рантайма — нарушение правила, которого на тот момент ещё не существовало. Этот пробел теперь закрыт обязательным правилом (см. docs/tehnicheskoe_zadanie.md, раздел 11): результат любого платного API-прогона должен проверяться и копироваться с эфемерного рантайма в той же ячейке, что его порождает. Повторный прогон, при идентичной конфигурации, воспроизвёл исходные числа с расхождением 1-3 п.п. по источникам (итоговая judge accuracy 0.760 против 0.768 у утраченного прогона) — в пределах остаточной API-стохастичности, уже задокументированной в проекте даже при temperature=0.0, а не признак ошибки в каком-либо из прогонов. Полный отчёт, включая таксономию оставшихся 60 ошибок: docs/tehnicheskoe_zadanie.md, раздел 14.
