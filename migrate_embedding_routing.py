...
- TAT-DQA's voyage-finance-2 vectors are ALREADY stored in the
  ...
  test_voyage_finance2_ab.py's earlier A/B run - overwriting
...
Prerequisite: test_voyage_finance2_ab.py must have already run against
...
repo root so `pipeline` is importable):
    !python migrate_embedding_routing.py
"""
...
def main() -> None:
    ...
    if missing_finance2:
        raise RuntimeError(
            ...
            f"test_voyage_finance2_ab.py against this collection first. First few: {missing_finance2[:5]}"
        )
